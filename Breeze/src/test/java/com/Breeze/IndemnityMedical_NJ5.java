...
public class IndemnityMedical_NJ5 extends BaseTest{
	
	@Test(priority=0)
    public void IndemnityMedical_NJ5_Claim() 
    {   
	    ...
	    try 
	    {
	    	...
	    	ncs.ClickTextName1("li", "New Jersey");
	    	TestListener.test.log(Status.PASS, "Verified that Jurisdiction dropdown is available and user selected the value 'New Jersey' from dropdown");
	    	ncs.sendKeys(ncs.InjuryDate, todayDate+"2019");
	    	...
	    	ncs.sendKeys(ncs.SearchField, "WC500-0070837-2019A");
	    	po.screenShotPass("User entereded the Policy Number 'WC500-0070837-2019A' in a search text field");
			...
			{				
				...
				Assert.assertTrue(PolicyNumber.contains("WC500-0070837-2019A"));
			...
			ncs.click(ncs.ClaimType);
			ncs.ClickTextName2("li", "Lost Time");
			ncs.click(ncs.ClaimAdministrator);
			ncs.ClickTextName2("li", "Stonewood Insurance Company");
			ncs.sendKeys(ncs.DateEmployerHadKnowledgeofInjury, todayDate+"2019");
			ncs.sendKeys(ncs.DateClaimAdministratorHadKnowledgeofInjury, todayDate+"2019");
			ncs.sendKeys(ncs.DateClaimAdministratorNotifiedOfEmployeeRepresentation, todayDate+"2019");
		
			ncs.click(ncs.Organization);
			ncs.ClickTextName1("li", "Email");
			ncs.sendKeys(ncs.InitialDateLastDayWorked,  todayDate+"2019");
			ncs.sendKeys(ncs.InitialReturnToWorkDate, todayDate+"2019");
			TestListener.test.log(Status.PASS, "User entered all the  mentioned details under 'General Information', 'Notice', 'Disablitiy Dates' sections. Claim Status- Open and Claim Type -Lost Time'");
			...
			po.screenShotPass("Ensured that system is navigated to 'Employee Details'");
			ncs.sendKeys(ncs.Height, "6");
			ncs.sendKeys(ncs.Weight, "88");
			ncs.click(ncs.Smoker);
			ncs.ClickTextName1("li", "No");
			TestListener.test.log(Status.PASS, "User added the information correctly for Height, Weight, Smoker and Hand Dominance");
			ncs.click(ncs.NextButton2);
			po.screenShotPass("Clicked on Next button");
			...
			Thread.sleep(1000);
			ncs.sendKeys(ncs.WageEffectiveDate,  todayDate+"2019");
			Thread.sleep(1000);
			ncs.sendKeys(ncs.WageEffectiveDate,  todayDate+"2019");
	    	...
	    	Assert.assertTrue(TotalReservedAmount.equals("$1,835.00") && TotalReserveAdjustment.equals("$1,835.00"));
	    	po.screenShotPass("Verified that after saving the record the Total Reserve amount and total reserve Adjustment amount becomes =1,835$");
	       	Thread.sleep(1000);
	    	...
	    	Assert.assertTrue(TotalReservedAmount1.equals("$1,841.00") && TotalReserveAdjustment1.equals("$1,841.00"));
	    	po.screenShotPass("Verified that after saving the record the Total Reserve amount and total reserve Adjustment amount becomes =1,835$+6= $1,841");
	    	...
	    	for(WebElement item : rows2_.subList( 0,1))
			{
				...
				Assert.assertTrue(incurredIndemnity.equals("$100.00") && ReserveBalanceIndemnity.equals("$100.00"));
				po.screenShotPass("Verified that Indemnity corresponding to Incurred column ,amount is '$10.00' and Indemnity corresponding to Reserve Balance column ,amount is '$100.00'");
			}
			...
			po.screenShotPass("Selected Provider, Paycode as 'Miscellaneous Expense'. Entered Service FromDate,Service To date, Scheduled Pay Date same as Date of Injury Date");
			Thread.sleep(1000);
	    	...
	    	for(WebElement item : rowss_5.subList( 0,1))
			{
				List <WebElement> columns = item.findElements(By.tagName("td"));
				String Status = columns.get(4).getText();
				Assert.assertTrue(Status.equals("Released"));
			...
			Thread.sleep(1000);
	       	...
	       	List <WebElement> rowss110 = ncs.PayableTable.findElements(By.xpath("//tr[contains(@id, 'ctl00_ctl00_ContentPlaceHolder1_ClaimsContent_m_pbPayables_i0_m_rgPayables_ctl00')]"));
	       	
	    	int sizes110 = rowss110.size();
	    	Assert.assertTrue(sizes110==3);
	    	for(WebElement item : rowss110.subList(0,1))
			{
				List <WebElement> columns = item.findElements(By.tagName("td"));
				int size1 = columns.size();
				String Status = columns.get(4).getText();
			...
			Thread.sleep(2000);
			//ncs.waitForElementToBeClickable(ncs.FinancialTable);
			List <WebElement> rows2_210 = ncs.FinancialTable.findElements(By.xpath("//tr[contains(@id, 'ctl00_ctl00_ContentPlaceHolder1_ClaimsContent_m_pbFinancialSummary_i0_m_ucSummary_m_rgFinancialSummary_ctl00')]"));
	    	for(WebElement item2 : rows2_210.subList( 1,2))
			{
				List <WebElement> columns = item2.findElements(By.tagName("td"));
				String incurredMedical = columns.get(1).getText();
				String PendingPayments = columns.get(5).getText();
			...
			po.screenShotPass("Selected Provider, Paycode as 'Penalties Paid'. Entered Service FromDate,Service To date, Scheduled Pay Date same as Date of Injury Date");
	    	...
	    	String PaidIndemnity = ncs.ElemenyByX("//*[@id=\"ctl00_ctl00_ContentPlaceHolder1_ClaimsContent_m_pbFinancialSummary_i0_m_ucSummary_m_rgFinancialSummary_ctl00__0\"]/td[9]").getText();
	    	...
	    	Assert.assertTrue(PaidIndemnity.equals("$60.00") && PaidMedical.equals("$100.00") && PaidExpense.equals("$50.00") && PaidOther.equals("$5.00"));
	    	po.screenShotPass("Also Verified that under Financial summary section: Under Net of Recoveries Corresponding to Paid  column: Indemnity amount is displayed as $60.00, Medical expense amount is displayed as $100.00, Expense amound displayed as 50 $ and Other Amount is displayed as $5.00");
